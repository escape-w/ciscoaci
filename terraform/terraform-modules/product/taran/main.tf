...
#ACI Native Bridge Domain Configuration
module "epg_bd_subnet" {
  source = "../../aci/epg-bd-subnet"
  for_each = {
    for v in var.epg_bd_subnet : "${v.vlan_id}" => v
  }
  tenant_dn                 = module.tenant[each.value.target_tenant].tenant_id
  vrf_dn                    = module.vrf[each.value.target_vrf].vrf_id
  anp_dn                    = module.application_profile[each.value.target_anp].anp_id
...
module "interafce_sub_policies" {
  source = "../../aci/interface-sub-policy"
...
module "vpc_security_group" {
  source = "../../aci/vpc-security"
  vpc_pg = var.vpc_pg
}

#EPG Static Paths
module "epg_static_paths" {
  source             = "../../aci/static-paths"
...
module "rul_subjects_filters" {
  source                       = "../../aci/contracts/contract-subjects-filters"
